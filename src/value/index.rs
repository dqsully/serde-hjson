...
/// A type that can be used to index into a `serde_hjson::Value`.
///
    ...
    I: Index,
{
    ...
    /// Index into a `serde_hjson::Value` using the syntax `value[0]` or
    /// `value["k"]`.
    ///
...
where
    I: Index,
{
    /// Write into a `serde_hjson::Value` using the syntax `value[0] = ...` or
    /// `value["k"] = ...`.
    ///
