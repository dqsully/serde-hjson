...
fn test_roundtrip_f64() {
    for &float in &[
        ...
        let json = serde_hjson::to_string(&float).unwrap();
        let output: f64 = serde_hjson::from_str(&json).unwrap();
...
fn test_roundtrip_f32() {
    ...
    let json = serde_hjson::to_string(&float).unwrap();
    let output: f32 = serde_hjson::from_str(&json).unwrap();
    assert_eq!(float, output);
}
...
fn test_malicious_number() {
    #[derive(Serialize)]
    #[serde(rename = "$serde_hjson::private::Number")]
    struct S {
        #[serde(rename = "$serde_hjson::private::Number")]
        f: &'static str,
    }

    let actual = serde_hjson::to_value(&S { f: "not a number" })
        .unwrap_err()
        .to_string();
...
fn test_deny_float_key() {
    ...
    assert!(serde_hjson::to_value(&map).is_err());
}
...
fn null_invalid_type() {
    let err = serde_hjson::from_str::<String>("null").unwrap_err();
    assert_eq!(
        format!("{}", err),
...
fn test_borrowed_raw_value() {
    ...
    let wrapper_from_str: Wrapper =
        serde_hjson::from_str(r#"{"a": 1, "b": {"foo": 2}, "c": 3}"#).unwrap();
    assert_eq!(r#"{"foo": 2}"#, wrapper_from_str.b.get());

    let wrapper_to_string = serde_hjson::to_string(&wrapper_from_str).unwrap();
    assert_eq!(r#"{"a":1,"b":{"foo": 2},"c":3}"#, wrapper_to_string);

    let wrapper_to_value = serde_hjson::to_value(&wrapper_from_str).unwrap();
    assert_eq!(json!({"a": 1, "b": {"foo": 2}, "c": 3}), wrapper_to_value);

    let array_from_str: Vec<&RawValue> =
        serde_hjson::from_str(r#"["a", 42, {"foo": "bar"}, null]"#).unwrap();
    ...
    let array_to_string = serde_hjson::to_string(&array_from_str).unwrap();
...
fn test_boxed_raw_value() {
    ...
    let wrapper_from_reader: Wrapper =
        serde_hjson::from_reader(br#"{"a": 1, "b": {"foo": 2}, "c": 3}"#.as_ref()).unwrap();
    assert_eq!(r#"{"foo": 2}"#, wrapper_from_reader.b.get());

    let wrapper_from_value: Wrapper =
        serde_hjson::from_value(json!({"a": 1, "b": {"foo": 2}, "c": 3})).unwrap();
    ...
    let array_from_reader: Vec<Box<RawValue>> =
        serde_hjson::from_reader(br#"["a", 42, {"foo": "bar"}, null]"#.as_ref()).unwrap();
